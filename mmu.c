...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
    ...
    while (current != NULL) {
        int free_size = current->blk->end - current->blk->start + 1;

        // Check if the block is large enough
        if (free_size >= blocksize) {
            ...
            // Update the free block to reflect the remaining size
            current->blk->start = allocated_block->end + 1;

            // If the free block is fully used, remove it
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
    ...
    while (current != NULL) {
        if (current->blk->pid == pid) {
            ...
            // Reset PID and add back to free list based on policy
            freed_block->pid = 0;
